<template>
  <view class="fish">
    <view class="count">
      <view class="count-num">{{vuex_fish.count}}</view>
...
export default {
  data () {
    return {
      ...
      interval: null,
      fishActive: false,
      audio: new Howl({ src: ['../../../static/video/sound.mp3'] }),
      bgmId: 0,
      bgm: new Howl({
        src: ['../../../static/video/bgm.mp3'],
        loop: true,
        autoplay: true
      })
    }
  ...
  methods: {
    ...
    clickMuyu () {
      const count = this.vuex_fish.count + 1
      this.fishActive = true
      this.tipsList.push(this.tabList[this.tabIndex].tips)
      setTimeout(() => {
